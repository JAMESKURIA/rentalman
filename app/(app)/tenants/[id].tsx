  ...
  useEffect(() => {
    const loadTenant = async () => {
      try {
        ...
        const tenantData = globalState.tenants.find(t => t.id === Number(id)).get();
        ...
        if (tenantData) {
          ...
          // Get house
          const houseData = globalState.houses.find(h => h.id === tenantData.houseId).get();
          if (houseData) {
            setHouse(houseData);
          ...
          const tenantFromDb = await databaseService.getTenantById(Number(id));
          ...
          if (tenantFromDb) {
            ...
            // Get house
            const houseFromDb = await databaseService.getHouseById(tenantFromDb.houseId);
            if (houseFromDb) {
        ...
        <>
          <View className="mb-6">
            <View className="flex-row justify-between items-center mb-4">
              ...
              >
                <Ionicons name="pencil-outline" size={20} color={isDarkMode ? '#9CA3AF' : '#6B7280'} />
              </TouchableOpacity>
            </View>
            ...
            <Card className="mb-4">
              <View className="space-y-2">
                <View className="flex-row justify-between">
                  ...
                  <View className={`px-2 py-0.5 rounded-full ${tenant.isActive ? 'bg-green-100' : 'bg-red-100'}`}>
                    <Text className={`text-xs ${tenant.isActive ? 'text-green-800' : 'text-red-800'}`}>
                ...
                <View className="flex-row justify-between">
                  <Text className={`${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Rent Due Day:</Text>
                  <Text className={`font-medium ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>
                    {tenant.rentDueDay}
                  </Text>
                </View>
