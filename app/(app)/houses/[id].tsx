...
import databaseService, { House, Tenant, HouseBill, ServiceRecord } from '../../../services/DatabaseService';
  ...
  useEffect(() => {
    const loadHouseDetails = async () => {
      try {
        ...
        const houseData = globalState.houses.find(h => h.id === Number(id)).get();
        
        if (houseData) {
          setHouse(houseData);
          
          // Load building
          const buildingData = globalState.buildings.find(b => b.id === houseData.buildingId).get();
          setBuilding(buildingData);
          
          // Load house type
          if (houseData.typeId) {
            const houseTypeData = globalState.houseTypes.find(t => t.id === houseData.typeId).get();
            setHouseType(houseTypeData);
          }
          
          // Load tenants
          const tenantsData = globalState.tenants.filter(t => t.houseId === Number(id)).get();
          setTenants(tenantsData);
          
          // Load bills
          const billsData = globalState.houseBills.filter(b => b.houseId === Number(id)).get();
          
          // Enrich bills with utility bill data
          const enrichedBills = [];
          for (const bill of billsData) {
            const utilityBill = globalState.utilityBills.find(ub => ub.id === bill.utilityBillId).get();
            if (utilityBill) {
              enrichedBills.push({
                ...bill,
          ...
          // Load services
          const servicesData = globalState.serviceRecords.filter(s => s.houseId === Number(id)).get();
          setServices(servicesData);
        } else {
          // If not in global state, fetch from database
          const houseFromDb = await databaseService.getHouseById(Number(id));
          
          if (houseFromDb) {
            setHouse(houseFromDb);
            
            // Load building
            const buildingFromDb = await databaseService.getBuildingById(houseFromDb.buildingId);
            setBuilding(buildingFromDb);
            
            // Load house type
            if (houseFromDb.typeId) {
              const houseTypeFromDb = await databaseService.getHouseTypeById(houseFromDb.typeId);
              setHouseType(houseTypeFromDb);
            }
            
            // Load tenants
            const tenantsFromDb = await databaseService.getTenantsByHouseId(Number(id));
            setTenants(tenantsFromDb);
            
            // Load bills
            const billsFromDb = await databaseService.getHouseBillsByHouseId(Number(id));
            setBills(billsFromDb);
            
            // Load services
            const servicesFromDb = await databaseService.getServiceRecordsByHouseId(Number(id));
      ...
      <ScrollView>
        ...
        <View className="mb-4">
          <View className="flex-row justify-between items-center mb-3">
            ...
            <Link href={{ pathname: "/tenants/add", params: { houseId: id, houseNumber: house.houseNumber } }} asChild>
              <Button
                title="Add Tenant"
        ...
        <View className="mb-4">
          <View className="flex-row justify-between items-center mb-3">
            ...
            <Link href={{ pathname: "/bills/add", params: { houseId: id } }} asChild>
              <Button
                title="Add Bill"
        ...
        <View className="mb-4">
          <View className="flex-row justify-between items-center mb-3">
            ...
            <Link href={{ pathname: "/services/add", params: { houseId: id } }} asChild>
              <Button
                title="Add Service"
                icon={<Ionicons name="add" size={16} color="white" />}
                size="sm"
