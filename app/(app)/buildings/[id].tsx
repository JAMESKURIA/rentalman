  ...
  useEffect(() => {
    const loadBuilding = async () => {
      try {
        ...
        const buildingData = globalState.buildings.find(b => b.id === Number(id)).get();
        ...
        if (buildingData) {
          ...
          const buildingFromDb = await databaseService.getBuildingById(Number(id));
        ...
        const housesData = globalState.houses.filter(h => h.buildingId === Number(id)).get();
    ...
    >
      <View className="flex-row justify-between items-center">
        ...
        <View className="flex-row items-center">
          <View className={`px-2 py-1 rounded-full mr-2 ${item.isOccupied ? 'bg-green-100' : 'bg-yellow-100'}`}>
    ...
    <Container>
      {isEditing ? (
        ...
        <View className="mb-6">
          ...
          <Card className="mb-4">
            <Text className={`font-medium mb-1 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>
              Address
            </Text>
            <Text className={`${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
              {building.address}
      ...
      <View className="flex-row justify-between items-center mb-4">
        ...
        <Link href={{ pathname: "/houses/add", params: { buildingId: id } }} asChild>
          <Button
            title="Add House"
